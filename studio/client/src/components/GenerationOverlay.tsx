...
export default function GenerationOverlay({ terrain = false }: { terrain?: boolean }) {
  const progress = useStudioStore((s) => s.generationProgress);
  const abort = useStudioStore((s) => s.abortGeneration);
  const terrainParams = useStudioStore((s) => s.terrainParams);

  const label = terrain
    ? `heightfield ${terrainParams.resolution}² + rocks`
    : progress
    ...
    <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-10">
      <div className="flex flex-col items-center gap-2 px-6 py-4 rounded-lg bg-space-panel border border-space-border">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-space-text">{terrain ? "Generating terrain..." : "Generating mesh..."}</span>
        </div>
